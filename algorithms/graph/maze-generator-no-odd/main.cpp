...
template<> struct Arbitrary<Dimension>
{
  static Gen<Dimension> arbitrary()
  {
    return gen::build<Dimension>(
        gen::set(&Dimension::width, gen::inRange(std::size_t(1), max_dim)),
        gen::set(&Dimension::height, gen::inRange(std::size_t(1), max_dim)));
  }
};
...
auto random_dimension()
{
  return rc::gen::apply([](Dimension d) { return Dimension{2*d.width+1, 2*d.height+1}; }
      , rc::gen::arbitrary<Dimension>());
}

auto random_point(Dimension const& dim)
{
  return rc::gen::apply([](std::size_t x, std::size_t y) { return Point{2*x,2*y}; }
      , rc::gen::inRange(std::size_t(), (dim.width-1)/2), rc::gen::inRange(std::size_t(), (dim.height-1)/2));
}

RC_GTEST_PROP(TEST_NAME, UniqueStart, (unsigned seed))
{
  auto dim = *random_dimension();
...
RC_GTEST_PROP(TEST_NAME, WallOnEachOddOdd, (unsigned seed))
{
  auto dim = *random_dimension();
  auto start_pt = *random_point(dim);
  auto end_pt = *random_point(dim);
  ...
  for (std::size_t j {1} ; j < dim.height ; j += 2)
  {
    for (std::size_t i {1} ; i < dim.width ; i += 2)
    {
      RC_ASSERT(array2d.data()[j][i] == to_char(MazeElement::Wall));
    }
  }
}

RC_GTEST_PROP(TEST_NAME, RoadOnEachEvenEven, (unsigned seed))
{
  auto dim = *random_dimension();
  auto start_pt = *random_point(dim);
  auto end_pt = *random_point(dim);
  ...
  for (std::size_t j {} ; j < dim.height ; j += 2)
  {
    for (std::size_t i {} ; i < dim.width ; i += 2)
    {
      RC_ASSERT(array2d.data()[j][i] != to_char(MazeElement::Wall));
    }
  }
